...
> 本笔记涵盖 C++ 泛型编程和 STL 核心技术，包括模板（函数模板/类模板）、STL 容器（string/vector/deque/stack/queue/list/set/map）、函数对象（仿函数/谓词）、常用算法（遍历/查找/排序/拷贝/算术/集合）等内容。学完本阶段后可通过演讲比赛流程管理系统进行实战练习。
